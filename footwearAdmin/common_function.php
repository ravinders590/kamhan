...
class Ecommerce extends Databaseconfig{
	...
	*/
	public function AddProduct($data){
		...
		$is_multiple_file = $this->MultipleImage($data);
		...
		$product_photo = serialize($data['product_photo']['name']);
		$product_photos = array('product_photo'=>$product_photo);
		$datas = array_merge($data,$slug_data,$youtube_videos,$product_photos);
		
		if ($is_multiple_file == 1) {
			$insert_data = $this->InsertDataNew('products',$datas);
			if ($insert_data == NULL) {
	...
	public function MultipleImage($data){
		
		$error=array();
		$extension=array("jpeg","jpg","png","gif");
		$multiple = $data['product_photo']['tmp_name'];
		foreach($multiple as $key=>$tmp_name) {
			
			$file_name=$data["product_photo"]["name"][$key];
			$file_tmp=$data["product_photo"]["tmp_name"][$key];
			$ext=pathinfo($file_name,PATHINFO_EXTENSION);

			if(in_array($ext,$extension)) {
				if(!file_exists("upload/".$file_name)) {
					move_uploaded_file($file_tmp=$_FILES["product_photo"]["tmp_name"][$key],"upload/".$file_name);
					
				}
				else {
					$filename=basename($file_name,$ext);
					$newFileName=$filename.time().".".$ext;
					move_uploaded_file($file_tmp=$_FILES["product_photo"]["tmp_name"][$key],"upload/".$newFileName);
				}
			}
			else {
	...
	*/
	...
	public function update_data($data,$id){
		...
		$product_photos = array('product_photo'=>$product_photo);
		$datas = array_merge($data,$slug_data,$youtube_videos,$product_photos);
		$result = $this->UpdateData('products',$datas,$id);
		if ($result == true) {
